...
每个工程团队都有一些成员，他们的编码智慧是无价的——命名约定、架构直觉、测试习惯，这些都塑造了代码库。当他们离开时，这些知识往往也随之而去。
...
├── examples/
│   ├── example-author-style.md           # 示例: React/TypeScript/HeroUI
│   ├── sample-summary.json               # 示例分析输出
│   └── sample-summary.zh.md              # 示例报告（中文）
└── evals/
    └── evals.json                        # 评估测试用例
```
